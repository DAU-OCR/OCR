...
def evaluate(model, val_loader, encoder, criterion):
    ...
    with torch.no_grad():
        for images, texts in val_loader:
            ...
            # loss 계산용 타겟
            targets = torch.tensor(encoder.encode(gt_text), dtype=torch.long).unsqueeze(0).to(device)
            target_lengths = torch.tensor([len(gt_text)], dtype=torch.long)
            input_lengths = torch.full(size=(1,), fill_value=outputs.size(0), dtype=torch.long).to(device)
            val_loss += criterion(outputs.log_softmax(2), targets.view(-1), input_lengths, target_lengths).item()
...
if __name__ == "__main__":
    ...
    try:
        for epoch in range(config.epochs):
            ...
            pd.DataFrame(pred_samples, columns=['GT', 'CTC_Pred', 'Beam_Pred', 'Char_Acc']) \
                .to_csv('train_logs/sample_predictions.csv', index=False)
            print("📄 예측 샘플 로그 저장됨: sample_predictions.csv")
